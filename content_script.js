...
class SyncUpSidebar {
  ...
  updateContextualCards(newCards) {
    ...
    if (!listContainer) {
      console.error('❌ contextual-cards-list element not found!');
    ...
    const generateCardHTML = (card, index) => {
      ...
      <div class="context-card ${card.expanded ? 'expanded' : ''}"
        ...
        <div class="card-body" style="display: ${card.expanded ? 'block' : 'none'}">
          ...
          ${card.useCase ? `
            <div class="card-section">
              <h5>${card.isChatboxAnswer ? 'Source' : 'Use Case'}</h5>
    ...
    console.log('📊 Cards visible in DOM:', listContainer.querySelectorAll('.context-card').length);
